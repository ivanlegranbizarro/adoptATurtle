...
<div class="container mx-auto mt-8">
  ...
  <form action="{{ route('adopciones.store') }}" method="POST">
    ...
    <div class="mb-4">
      ...
      <textarea name="motivation" id="motivation" rows="4" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm" placeholder="Why do you want to adopt {{ $tortuga->name }}? Please, provide as much information as possible. We need to know {{ $tortuga->name }} will be in good hands.">@if(old('motivation')){{ old('motivation') }}@endif</textarea>
      @error('motivation')
